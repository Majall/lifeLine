<?php
require 'header.php';
include 'CRUD/edit_hospital_process.php';


$query = "SELECT *
     FROM hospital 
     WHERE Hospital_id='$_SESSION[userID]'";
$result_hospital = mysqli_query($conn, $query);
$row = mysqli_fetch_assoc($result_hospital);

$Email = $row['Email'];
$Phone = $row['Tel_no'];
$Password = $row['H_Password'];

if (!$result_hospital) {

  die('Error: ' . mysqli_error($conn));
}
?>
<!DOCTYPE html>
<html>

<head>
  <link rel="stylesheet" href="css/hospital_edit.css" />
</head>

<body>
  <main class="frame">
    <div class="sec1">
      <div class="row1">
        <div class="blue">
          <?php
          echo "<h2 class=\"blue\">" . $_SESSION['username'] . "</h2>";
          ?></div>
        <div class="head">
          <?php echo "<h5 class=\"head\">" . $_SESSION['email'] . "</h5>"; ?>
        </div>

      </div>
    </div>
    <br />
    <hr />
    <form method="post">
    <div class="but1">
            <input type="hidden" name="HospitalID" value="<?php echo $_SESSION['userID'] ?>">
                <input class="del" type="submit" name="delete" value="Delete Profile" onclick="return confirm('Are you Sure ?')">
        </div>
      <div class="sec1">
        <div class="one">
          <label for="email">Email: </label>
          <input type="email" name="email" value="<?php echo $Email; ?>" required>
        </div>
        <div class="one">
          <label for="phone">Phone Number: </label>
          <input type="text" name="number" value="<?php echo $Phone; ?>" required>
        </div>
      </div>

      <input type="hidden" name="HospitalID" value="<?php echo $_SESSION['userID'] ?>">
      <input class="update" type="submit" name="update" value="Update">
    </form>
    <br>
    <hr>
    <br>
    <form method="post" onsubmit="return validateForm()">
      <div class="sec2">
        <div class="sec">
          <label for="password">New Password: </label>
          <input type="password" name="password" id="password" required>
        </div>
        <div class="sec">
          <label for="cPassword">Confirm Password: </label>
          <input type="password" name="cPassword" id="cPassword" required>
        </div>
      </div>
      <input class="change" type="submit" name="change" value="Change">
    </form>
  </main>

  <script>
    function validateForm() {

    
      var password = document.getElementById("password").value;
      var confirmPassword = document.getElementById("cPassword").value;
      if (password != confirmPassword) {
        alert("Passwords do not match.");
        return false;
      }

   
      var passwordRegex = /^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
      if (!passwordRegex.test(password)) {
        alert("Password must contain at least 8 characters including one uppercase letter, one special character, and one digit.");
        return false;
      }

      return true;
    }
  </script>
</body>
<?php
require 'footer.php';
?>

</html>